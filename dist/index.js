...
async function gitlabRequest(path, { method = "GET", body, params } = {}) {
    ...
    if (!response.ok) {
        ...
        throw new types_js_1.McpError(types_js_1.ErrorCode.InternalError, `GitLab API error ${response.status}: ${detail || response.statusText}`);
    }
    // 204 No Content
...
function resolveGroupScope(group) {
...
async function searchProjects(search, { group, perPage = 20, page = 1 } = {}) {
    ...
    if (scope) {
        const projects = await gitlabRequest(`/groups/${scope.encoded}/projects`, {
            params: {
                ...params,
    ...
    const projects = await gitlabRequest("/projects", {
        params,
    });
    return { projects, scopeLabel: "accessible projects" };
}
async function listAccessibleProjectsCatalog(forceRefresh = false) {
    const now = Date.now();
...
function extractProjectPathFromGitLabUrl(value) {
...
function canonicalizeProjectInput(value) {
...
function tokenizeProjectLookup(value) {
    return normalizeProjectLookup(value).split(" ").filter(Boolean);
}
async function searchProjectsForResolution(query) {
...
function pickResolvedProject(projects, query) {
    const canonicalQuery = canonicalizeProjectInput(query);
    const normalizedQuery = normalizeProjectLookup(canonicalQuery);
    const pathNamespaceMatch = projects.filter((project) => project.path_with_namespace.toLowerCase() === canonicalQuery.toLowerCase());
    if (pathNamespaceMatch.length === 1)
        return pathNamespaceMatch[0];
    const pathMatch = projects.filter((project) => project.path.toLowerCase() === canonicalQuery.toLowerCase());
    if (pathMatch.length === 1)
        return pathMatch[0];
    const nameMatch = projects.filter((project) => project.name.toLowerCase() === canonicalQuery.toLowerCase());
    if (nameMatch.length === 1)
        return nameMatch[0];
    const normalizedPathNamespaceMatch = projects.filter((project) => normalizeProjectLookup(project.path_with_namespace) === normalizedQuery);
    if (normalizedPathNamespaceMatch.length === 1)
        return normalizedPathNamespaceMatch[0];
    const normalizedPathMatch = projects.filter((project) => normalizeProjectLookup(project.path) === normalizedQuery);
    if (normalizedPathMatch.length === 1)
        return normalizedPathMatch[0];
    const normalizedNameMatch = projects.filter((project) => normalizeProjectLookup(project.name) === normalizedQuery);
    if (normalizedNameMatch.length === 1)
        return normalizedNameMatch[0];
    const queryTokens = tokenizeProjectLookup(canonicalQuery);
    const pathNamespaceSequenceMatch = projects.filter((project) => hasTokenSequence(tokenizeProjectLookup(project.path_with_namespace), queryTokens));
    if (pathNamespaceSequenceMatch.length === 1)
        return pathNamespaceSequenceMatch[0];
    const pathSequenceMatch = projects.filter((project) => hasTokenSequence(tokenizeProjectLookup(project.path), queryTokens));
    if (pathSequenceMatch.length === 1)
        return pathSequenceMatch[0];
    const nameSequenceMatch = projects.filter((project) => hasTokenSequence(tokenizeProjectLookup(project.name), queryTokens));
    if (nameSequenceMatch.length === 1)
        return nameSequenceMatch[0];
...
function isGitLabStatusError(error, status) {
    return error instanceof types_js_1.McpError && error.message.startsWith(`GitLab API error ${status}:`);
}
function renderIssueDetails(issue, project, workItemStatus) {
    ...
    lines.push(`**Author:** ${issue.author.name} (@${issue.author.username})`, `**Assignees:** ${assignees}`, `**Labels:** ${issue.labels.join(", ") || "none"}`, `**Milestone:** ${issue.milestone?.title ?? "none"}`, `**Due date:** ${issue.due_date ?? "none"}`, `**Weight:** ${issue.weight ?? "none"}`, `**Created:** ${issue.created_at}`, `**Updated:** ${issue.updated_at}`, `**Closed:** ${issue.closed_at ?? "—"}`, `**URL:** ${issue.web_url}`, "", "### Description", issue.description || "_No description provided._");
    return lines.join("\n");
}
...
async function findIssueInMatchingProjects(projectQuery, iid, excludedProject) {
    ...
    for (const candidate of candidates) {
        try {
            const issue = await gitlabRequest(`/projects/${candidate.id}/issues/${iid}`);
...
async function resolveProject(project, { persist = false } = {}) {
    const value = project?.trim();
    if (!value) {
        if (activeProject) {
            return activeProject;
        }
        throw new types_js_1.McpError(types_js_1.ErrorCode.InvalidParams, "Project is required. Pass `project` (for example `group-name/project-name`, `group-name project-name`, or `project-name`) or call `select_project` first.");
    }
    ...
    if (!resolved) {
        const suggestions = projects
            ...
            .join(", ");
        if (projects.length === 0) {
            throw new types_js_1.McpError(types_js_1.ErrorCode.InvalidParams, `Project "${canonicalValue}" was not found in ${scopeLabel}. Use \`list_projects\` or pass the full namespace/path.`);
        }
        throw new types_js_1.McpError(types_js_1.ErrorCode.InvalidParams, `Project "${canonicalValue}" is ambiguous in ${scopeLabel}. Matching projects: ${suggestions}. Pass the full namespace/path.`);
    }
    const resolvedProject = {
...
function getNumericIdentifier(args, keys, label) {
    ...
    throw new types_js_1.McpError(types_js_1.ErrorCode.InvalidParams, `${label} is required. Pass one of: ${keys.map((key) => `\`${key}\``).join(", ")}.`);
}
function normalizePerPage(value, fallback = 20) {
    if (typeof value !== "number" || !Number.isFinite(value)) {
        return fallback;
    }
    return Math.min(100, Math.max(1, Math.trunc(value)));
}
function parseRequestedLabels(labels) {
    if (!labels)
        return [];
    const values = labels
        .split(",")
...
function normalizeLabelLookup(value) {
...
async function listProjectLabels(project) {
    const labels = [];
    let page = 1;
    const perPage = 100;
    while (true) {
        const batch = await gitlabRequest(`/projects/${project.encoded}/labels`, {
            params: {
                per_page: perPage,
                page,
...
async function resolveRequestedIssueLabels(project, labels) {
    ...
    for (const requested of requestedLabels) {
        ...
        const exactInsensitiveMatches = availableLabels.filter((label) => normalizeLabelLookup(label.name) === normalizedRequested);
        if (exactInsensitiveMatches.length === 1) {
            resolvedLabels.push({ requested, resolved: exactInsensitiveMatches[0].name });
            continue;
        }
        if (exactInsensitiveMatches.length > 1) {
            throw new types_js_1.McpError(types_js_1.ErrorCode.InvalidParams, `Label "${requested}" is ambiguous in ${project.raw}. Matching labels: ${exactInsensitiveMatches
                .map((label) => label.name)
                .join(", ")}.`);
        }
        const queryTokens = tokenizeLabel(requested);
        const tokenMatches = availableLabels.filter((label) => hasTokenSequence(tokenizeLabel(label.name), queryTokens));
        if (tokenMatches.length === 1) {
            resolvedLabels.push({ requested, resolved: tokenMatches[0].name });
            continue;
        }
        if (tokenMatches.length > 1) {
            throw new types_js_1.McpError(types_js_1.ErrorCode.InvalidParams, `Label "${requested}" is ambiguous in ${project.raw}. Matching labels: ${tokenMatches
                .map((label) => label.name)
                .join(", ")}. Pass the full label name.`);
        }
        throw new types_js_1.McpError(types_js_1.ErrorCode.InvalidParams, `Label "${requested}" was not found in ${project.raw}. Pass the full label name used in GitLab.`);
    }
    return resolvedLabels;
}
function issueHasAllRequestedLabels(issueLabels, requestedLabels) {
...
async function gitlabGraphQL(query, variables) {
    ...
    if (!response.ok) {
        ...
        throw new types_js_1.McpError(types_js_1.ErrorCode.InternalError, `GitLab GraphQL error ${response.status}: ${detail || response.statusText}`);
    }
    const result = (await response.json());
    if (result.errors && result.errors.length > 0) {
        throw new types_js_1.McpError(types_js_1.ErrorCode.InternalError, `GitLab GraphQL errors: ${result.errors.map((e) => e.message).join(", ")}`);
    }
    return result.data;
}
...
`;
async function fetchWorkItemStatus(projectFullPath, iid) {
    try {
        const data = await gitlabGraphQL(WORK_ITEM_STATUS_QUERY, { fullPath: projectFullPath, iids: [String(iid)] });
        const workItem = data?.project?.workItems?.nodes?.[0];
        if (!workItem)
            return null;
        const statusWidget = workItem.widgets.find((w) => "status" in w && w.status != null);
        return {
            iid: workItem.iid,
...
function getAuditModelLabel(meta) {
...
function addAiAuditHeader(text, meta) {
    ...
    return [
        "<!-- gitlab-mcp-ai-audit -->",
        "> [!CAUTION]",
        "> **AI-generated — not a final engineering response.**",
        "> This content was produced automatically and has not been reviewed or approved by the engineering team.",
        "> Do not use this as the basis for a client-facing commitment or estimate without explicit sign-off.",
        `> Model: ${getAuditModelLabel(meta)} | Client: ${getAuditClientLabel()} | ${new Date().toISOString()}`,
        "",
        text,
    ].join("\n");
}
    ...
    resources: [
        {
            ...
            description: "Catalog of GitLab projects accessible to the configured token. Use it to map natural repository names to full namespace paths before calling tools.",
...
server.setRequestHandler(types_js_1.ListToolsRequestSchema, async () => ({
    tools: [
        ...
        {
            name: "select_project",
            description: "Resolve and store the active GitLab project for this MCP session. Use this when the user wants to switch repositories for follow-up requests.",
            inputSchema: {
                type: "object",
                properties: {
        ...
        },
        {
            ...
            description: "Clear the active GitLab project stored for this MCP session so later tool calls must pass `project` explicitly or call `select_project` again.",
            inputSchema: {
                type: "object",
                properties: {},
            },
        ...
        {
            name: "list_projects",
            description: "List projects available to the authenticated user. Optionally scope discovery to a specific GitLab group/organization.",
            inputSchema: {
                type: "object",
                properties: {
                    search: {
        ...
        {
            name: "list_labels",
            description: `List labels available in a project. Use this to inspect exact GitLab label names before filtering issues. ${explicitProjectGuidance}`,
            inputSchema: {
                type: "object",
                properties: {
                    project: projectInputSchema,
        ...
        },
        {
            ...
            description: `List merge requests for a project. Defaults to open MRs and can filter by state and assignee username. ${explicitProjectGuidance}`,
            inputSchema: {
                type: "object",
                properties: {
                    project: projectInputSchema,
        ...
        },
        {
            ...
            description: `Post a comment on a specific merge request. Comments created through this MCP are automatically marked as AI-generated for audit purposes. ${explicitProjectGuidance}`,
            inputSchema: {
                type: "object",
                properties: {
                    project: projectInputSchema,
        ...
        {
            name: "list_issues",
            description: `List issues for a project. Optionally filter by state and labels. ${explicitProjectGuidance}`,
            inputSchema: {
                type: "object",
                properties: {
                    project: projectInputSchema,
                    ...
                    labels: {
                        type: "string",
                        description: "Comma-separated list of label names to filter by. Exact label-name matches are case-insensitive, unique shorthands like `blocked` can resolve to `Status Blocked`, and issues must contain all resolved labels.",
                    },
                    assignee_username: {
                        type: "string",
                        description: "Filter by assignee GitLab username",
        ...
        {
            name: "get_issue",
            description: `Get full details of a specific GitLab issue or ticket, including title, description, labels, assignees, and metadata. ${explicitProjectGuidance}`,
            inputSchema: {
                type: "object",
                properties: {
                    project: projectInputSchema,
        ...
        },
        {
            ...
            description: `Get all comments and notes on a specific GitLab issue or ticket, in chronological order. ${explicitProjectGuidance}`,
            inputSchema: {
                type: "object",
                properties: {
                    project: projectInputSchema,
        ...
        },
        {
            ...
            description: `Post a comment on a specific GitLab issue or ticket. Comments created through this MCP are automatically marked as AI-generated for audit purposes. ${explicitProjectGuidance}`,
            inputSchema: {
                type: "object",
                properties: {
                    project: projectInputSchema,
        ...
        {
            name: "update_issue",
            description: `Update an existing GitLab issue or ticket. Any description written through this MCP is automatically marked as AI-generated for audit purposes. ${explicitProjectGuidance}`,
            inputSchema: {
                type: "object",
                properties: {
                    project: projectInputSchema,
        ...
        },
        {
            ...
            description: `List all work items in a project with their Work Item Status in a single call. Uses the GitLab GraphQL Work Items API because the status field is not available in the REST API. Useful for getting a full status overview of all tickets in a project. ${explicitProjectGuidance}`,
            inputSchema: {
                type: "object",
                properties: {
                    project: projectInputSchema,
        ...
        },
        {
            ...
            description: `Get the Work Item status of a specific GitLab issue or ticket (e.g. "In Dev Review", "Ready for QA"). Uses the GraphQL Work Items API because the status field is not available in the REST API. ${explicitProjectGuidance}`,
            inputSchema: {
                type: "object",
                properties: {
                    project: projectInputSchema,
        ...
        {
            name: "create_issue",
            description: `Create a new issue in a project. Any description written through this MCP is automatically marked as AI-generated for audit purposes. ${explicitProjectGuidance}`,
            inputSchema: {
                type: "object",
                properties: {
                    project: projectInputSchema,
...
server.setRequestHandler(types_js_1.CallToolRequestSchema, async (request) => {
    ...
    try {
        switch (name) {
            ...
            // ── list_labels ──────────────────────────────────────────────────────
            case "list_labels": {
                const { project: projectArg, search } = args;
                const project = await resolveProject(projectArg, { persist: true });
                const normalizedSearch = search ? normalizeLabelLookup(search) : undefined;
                const labels = (await listProjectLabels(project))
                    .filter((label) => normalizedSearch ? normalizeLabelLookup(label.name).includes(normalizedSearch) : true)
                    .sort((a, b) => a.name.localeCompare(b.name, undefined, { sensitivity: "base" }));
                if (labels.length === 0) {
                    return textResult(search
            ...
            case "list_merge_requests": {
                const { project: projectArg, state = "opened", assignee_username, per_page = 20 } = args;
                const project = await resolveProject(projectArg, { persist: true });
                const mrs = await gitlabRequest(`/projects/${project.encoded}/merge_requests`, {
                    params: {
                        state,
                        assignee_username,
                        per_page,
            ...
            case "get_merge_request_diff": {
                const { project: projectArg, iid } = args;
                const project = await resolveProject(projectArg, { persist: true });
                const diffs = await gitlabRequest(`/projects/${project.encoded}/merge_requests/${iid}/diffs`);
                if (diffs.length === 0) {
            ...
            case "get_merge_request_comments": {
                const { project: projectArg, iid } = args;
                const project = await resolveProject(projectArg, { persist: true });
                const notes = await gitlabRequest(`/projects/${project.encoded}/merge_requests/${iid}/notes`, { params: { sort: "asc", order_by: "created_at", per_page: 100 } });
                const userNotes = notes.filter((n) => !n.system);
                if (userNotes.length === 0) {
                    return textResult(`No comments on MR !${iid} yet.`);
                }
                const lines = userNotes.map((n) => `**${n.author.username}** (${n.created_at}):\n${n.body}`);
                return {
                    content: [
                        {
                            type: "text",
            ...
            case "add_merge_request_comment": {
                const { project: projectArg, iid, body } = args;
                const project = await resolveProject(projectArg, { persist: true });
                const auditedBody = addAiAuditHeader(body, request.params._meta) ?? body;
                const note = await gitlabRequest(`/projects/${project.encoded}/merge_requests/${iid}/notes`, { method: "POST", body: { body: auditedBody } });
                return {
                    content: [
                        {
                            type: "text",
            ...
            // ── list_pipelines ───────────────────────────────────────────────────
            case "list_pipelines": {
                const { project: projectArg, status, ref, per_page = 20 } = args;
                const project = await resolveProject(projectArg, { persist: true });
                const pipelines = await gitlabRequest(`/projects/${project.encoded}/pipelines`, {
                    params: {
                        status,
                        ref,
                        per_page,
            ...
            case "get_pipeline_jobs": {
                const { project: projectArg, pipeline_id } = args;
                const project = await resolveProject(projectArg, { persist: true });
                const jobs = await gitlabRequest(`/projects/${project.encoded}/pipelines/${pipeline_id}/jobs`, { params: { per_page: 100 } });
                if (jobs.length === 0) {
            ...
            // ── list_issues ──────────────────────────────────────────────────────
            case "list_issues": {
                const { project: projectArg, state = "opened", labels, assignee_username, per_page = 20 } = args;
                const project = await resolveProject(projectArg, { persist: true });
                ...
                const issues = [];
                if (requestedLabels.length === 0) {
                    issues.push(...(await gitlabRequest(`/projects/${project.encoded}/issues`, {
                        params: {
                            state,
                            assignee_username,
                    ...
                    while (issues.length < requestedPerPage) {
                        const batch = await gitlabRequest(`/projects/${project.encoded}/issues`, {
                            params: {
                                state,
                                assignee_username,
                                per_page: batchSize,
                                ...
                                ...(resolvedLabelMappings.length > 0
                                    ? [
                                        `**Labels filter:** ${resolvedLabelMappings
                                            .map((mapping) => mapping.requested === mapping.resolved
                                            ? mapping.resolved
            ...
            case "get_issue": {
                const { project: projectArg } = args;
                const iid = getNumericIdentifier(args, ["iid", "issue", "ticket"], "Issue IID");
                const project = await resolveProject(projectArg, { persist: false });
                let resolvedProject = project;
                let issue;
                try {
                    issue = await gitlabRequest(`/projects/${project.encoded}/issues/${iid}`);
                }
                catch (error) {
                    if (isGitLabStatusError(error, 404) && projectArg) {
                        ...
                        if (matches.length === 1) {
                            ...
                            return textResult(`Issue #${iid} was not found in ${project.raw}, but it exists in multiple matching projects: ${matches
                                .map((match) => match.project.raw)
                                .join(", ")}. Pass the full namespace/path.`);
                        }
                        else {
                            return textResult(`Issue #${iid} was not found in ${project.raw}. If you meant a different matching project, pass the full namespace/path explicitly.`);
                        }
                    }
                    else if (isGitLabStatusError(error, 404)) {
                        return textResult(`Issue #${iid} was not found in the active project ${project.raw}. If you meant another repository, pass \`project\` explicitly or call \`select_project\` first.`);
                    }
                    else {
                        throw error;
                    }
            ...
            case "get_issue_comments": {
                ...
                let resolvedProject = project;
                try {
                    await gitlabRequest(`/projects/${project.encoded}/issues/${iid}`);
                }
                catch (error) {
                    if (isGitLabStatusError(error, 404) && projectArg) {
                        const matches = await findIssueInMatchingProjects(projectArg, iid, project.raw);
                ...
                setActiveProject(resolvedProject);
                const notes = await gitlabRequest(`/projects/${resolvedProject.encoded}/issues/${iid}/notes`, { params: { sort: "asc", order_by: "created_at", per_page: 100 } });
                const userNotes = notes.filter((n) => !n.system);
                if (userNotes.length === 0) {
            ...
            case "add_issue_comment": {
                const { project: projectArg, body } = args;
                const iid = getNumericIdentifier(args, ["iid", "issue", "ticket"], "Issue IID");
                const project = await resolveProject(projectArg, { persist: true });
                const auditedBody = addAiAuditHeader(body, request.params._meta) ?? body;
                const note = await gitlabRequest(`/projects/${project.encoded}/issues/${iid}/notes`, { method: "POST", body: { body: auditedBody } });
                return textResult(`✅ Comment posted on issue #${iid} in ${project.raw} by ${note.author.username} at ${note.created_at} (note ID: ${note.id}).`);
            }
            // ── update_issue ─────────────────────────────────────────────────────
            case "update_issue": {
                const { project: projectArg, title, description, labels, add_labels, remove_labels, assignee_ids, due_date, weight, state_event, } = args;
                const iid = getNumericIdentifier(args, ["iid", "issue", "ticket"], "Issue IID");
                const project = await resolveProject(projectArg, { persist: true });
                const hasUpdates = [
                    title,
                ...
                if (!hasUpdates) {
                    throw new types_js_1.McpError(types_js_1.ErrorCode.InvalidParams, "No issue updates were provided. Pass at least one field such as `title`, `description`, `labels`, `assignee_ids`, `due_date`, `weight`, or `state_event`.");
                }
                const auditedDescription = addAiAuditHeader(description, request.params._meta);
                const issue = await gitlabRequest(`/projects/${project.encoded}/issues/${iid}`, {
                    method: "PUT",
                    body: {
                        title,
            ...
            case "list_work_item_statuses": {
                const { project: projectArg, per_page = 100 } = args;
                const project = await resolveProject(projectArg, { persist: true });
        ...
        `;
                const data = await gitlabGraphQL(query, { fullPath: project.raw, first: limit });
                const nodes = data?.project?.workItems?.nodes ?? [];
                if (nodes.length === 0) {
                    return textResult(`No work items found in ${project.raw}.`);
                }
                const lines = nodes.map((item) => {
                    const statusWidget = item.widgets.find((w) => "status" in w && w.status != null);
            ...
            case "get_issue_status": {
                ...
                const iid = getNumericIdentifier(args, ["iid", "issue", "ticket"], "Issue IID");
                const project = await resolveProject(projectArg, { persist: false });
                const statusInfo = await gitlabGraphQL(WORK_ITEM_STATUS_QUERY, { fullPath: project.raw, iids: [String(iid)] });
                const workItem = statusInfo?.project?.workItems?.nodes?.[0];
                if (!workItem) {
                    return textResult(`Work item #${iid} was not found in ${project.raw} via the GraphQL Work Items API. Make sure the issue exists and the Work Items feature is enabled for this project.`);
                }
                const statusWidget = workItem.widgets.find((w) => "status" in w && w.status != null);
            ...
            // ── create_issue ─────────────────────────────────────────────────────
            case "create_issue": {
                const { project: projectArg, title, description, labels, assignee_ids } = args;
                const project = await resolveProject(projectArg, { persist: true });
                const auditedDescription = addAiAuditHeader(description, request.params._meta);
                const issue = await gitlabRequest(`/projects/${project.encoded}/issues`, {
                    method: "POST",
                    body: {
                        title,
                        description: auditedDescription,
            ...
            case "get_file_contents": {
                const { project: projectArg, file_path, ref = "main" } = args;
                const project = await resolveProject(projectArg, { persist: true });
                const file = await gitlabRequest(`/projects/${project.encoded}/repository/files/${encodeURIComponent(file_path)}`, { params: { ref } });
            ...
            // ── list_branches ─────────────────────────────────────────────────────
            case "list_branches": {
                const { project: projectArg, search, per_page = 20 } = args;
                const project = await resolveProject(projectArg, { persist: true });
                const branches = await gitlabRequest(`/projects/${project.encoded}/repository/branches`, { params: { search, per_page, order_by: "updated", sort: "desc" } });
        ...
        if (error instanceof types_js_1.McpError && error.code !== types_js_1.ErrorCode.MethodNotFound) {
            return textResult(error.message);
        }
        if (error instanceof types_js_1.McpError)
            throw error;
        const message = error instanceof Error ? error.message : String(error);
        throw new types_js_1.McpError(types_js_1.ErrorCode.InternalError, `Unexpected error: ${message}`);
    }
});
// ---------------------------------------------------------------------------
// Start server
...
async function main() {
    ...
    process.stderr.write(`GitLab MCP server running (active project: ${activeProject?.raw ?? "none"}, base: ${GITLAB_BASE_URL})\n`);
}
main().catch((err) => {
